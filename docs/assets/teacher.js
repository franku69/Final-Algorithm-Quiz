...
(function () {
  'use strict';
  var C = window.PacketCore, API = window.PacketAPI, esc = C.escapeHTML, cfg = window.PACKET_CONFIG, TOTAL = window.PACKET_CONFIG.questionCount || 40;
  ...
  function paintDashboard() {
    ...
    rows.forEach(function (s) {
      ...
      if (!tr) { tr = document.createElement('tr'); tr.dataset.attempt = s.attemptId; tr.setAttribute('role','row'); ['Student','Status','Progress','Score','Last activity','Device help'].forEach(function (label) { var td = document.createElement('td'); td.dataset.label = label; td.setAttribute('role','cell'); tr.appendChild(td); }); rowNodes.set(s.attemptId,tr); tbody.appendChild(tr); }
      ...
      tdSet(tr.children[2], '<strong>' + s.answered + ' / ' + TOTAL + ' saved</strong><div class="mini-progress"><span style="width:' + Math.min(100, s.answered / TOTAL * 100) + '%"></span></div><small>' + (s.submitted ? 'Completed' : (s.currentPosition ? 'Viewing item ' + s.currentPosition : 'Waiting / reviewing')) + '</small>');
    ...
    if (!rows.length) { if (!empty) { empty=document.createElement('tr');empty.id='empty-students';empty.className='empty-table';empty.setAttribute('role','row');empty.innerHTML='<td role="cell" colspan="6">Share the room link. Students will appear here automatically as they join.</td>';tbody.appendChild(empty); } }
  ...
  document.addEventListener('click', async function(e){
    ...
    if(action==='answer-key'){
      try{var data=await call('teacher_bank');var lines=[data.bank.title,'TEACHER ANSWER KEY — KEEP PRIVATE','Options are shuffled independently for each student. Match the answer by text; the source option ID below is from the supplied PDF.',''];data.bank.questions.forEach(function(q){lines.push(q.number+'. ['+q.topic+'] '+q.prompt,'Source PDF option: '+q.correct+' — Correct answer: '+q.options.find(function(o){return o.id===q.correct;}).text,'Source: CS111 Topic 5 Algorithms 40-question quiz PDF',q.explanation,'');});C.download('CS111_Topic5_Algorithms_Answer_Key.txt',lines.join('\n'));}catch(err){showError(err.message);}return;
    }
