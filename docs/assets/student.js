/* Packet Quest GitHub Live v7.2: one serial sync queue, server-only grading, durable drafts. */
(function () {
  'use strict';
  var C = window.PacketCore, API = window.PacketAPI, cfg = window.PACKET_CONFIG, TOTAL = window.PACKET_CONFIG.questionCount || 40;
  ...
  function visualHTML(q) {
    var v = q && q.visual ? q.visual : {};
    var diagrams = {
      'iteration-loop': ['./assets/algorithm-loop.jpg', 1365, 456],
      'insertion-sort': ['./assets/algorithm-insertion-sort.jpg', 1365, 456],
      'factorial-return': ['./assets/algorithm-factorial-return.jpg', 1365, 456],
      'binary-search': ['./assets/algorithm-binary-search.jpg', 1365, 456],
      'chain-cut': ['./assets/algorithm-chain-cut.jpg', 1365, 456],
      'flowchart-symbols': ['./assets/algorithm-flowchart-symbols.png', 1200, 260]
    };
    var diagram = diagrams[String(v.diagram || '')];
    if (!diagram) return '';
    var alt = v.alt || 'Algorithm diagram';
    return '<figure class="question-diagram" role="group" aria-label="Algorithm diagram"><img src="' + diagram[0] + '" width="' + diagram[1] + '" height="' + diagram[2] + '" alt="' + esc(alt) + '"><figcaption><span>' + esc(v.caption || 'Topic 5 diagram') + '</span><details class="diagram-expand"><summary class="diagram-open">VIEW FULL SIZE</summary><div class="diagram-fullsize" role="region" aria-label="Full-size diagram"><img src="' + diagram[0] + '" width="' + diagram[1] + '" height="' + diagram[2] + '" alt="Full size: ' + esc(alt) + '"></div></details></figcaption></figure>';
  }
  function renderQuiz() {
