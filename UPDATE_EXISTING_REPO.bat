...
if not exist .git (echo This folder is not the cloned Git repository. Copy these updated files into your existing Final-Networking-Quiz folder first.& pause & exit /b 1)
...
if exist Packet_Quest_Topic5_BANK_KEY_PRIVATE.txt (echo ERROR: private bank key file is inside the repo. Move it out before continuing.& pause & exit /b 1)
echo Checking staged changes...
git add -A
git status
choice /C YN /M "Commit and push these changes now"
if errorlevel 2 exit /b 0
git commit -m "Update quiz to CS111 Topic 5 Algorithms"
