...
class QuizStore:
    ...
    def __init__(self, private_dir=None, bank_path=None, teacher_key=None, clock=now_ms):
        ...
        declared_count = self.bank.get('count')
        if type(declared_count) is not int or len(self.questions) != declared_count or len(self.questions) != 40 or any(len(q['options']) != 4 or q['correct'] not in {o['id'] for o in q['options']} for q in self.questions.values()):
            raise RuntimeError('The decrypted source question bank is invalid. This build requires the matching 40-question Topic 5 bank and private key.')
        self.clock = clock
    ...
    def _dispatch(self, data, token):
        ...
        if action == 'teacher_login':
            if not isinstance(token, str) or not hmac.compare_digest(token.encode('utf-8'), self.key.encode('utf-8')):
                raise ApiError('Teacher key not accepted. Check your private PACKET_TEACHER_KEY setting.', 401, 'teacher_login')
